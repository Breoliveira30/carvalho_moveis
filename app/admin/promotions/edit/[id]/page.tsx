...
import { getProducts, getPromotions, updatePromotion, type Product } from "@/lib/supabase"
...
export default function EditPromotion({ params }: EditPromotionPageProps) {
  ...
  const loadData = async () => {
    ...
    const promotion = promotionsData.find((p) => p.id === params.id)
    if (!promotion) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await updatePromotion(params.id, promotionData)
      if (result) {
        router.push("/admin/promotions")
      }
    } catch (error) {
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-2xl mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-bold text-gray-900">Editar Promoção</h1>
        </div>
