...
export default function ProductCarousel({ images, productName }: ProductCarouselProps) {
  ...
  // Auto-play opcional (comentado por enquanto)
  // useEffect(() => {
  //   if (images.length > 1) {
  ...
  return (
    <div className="relative w-full">
      {/* Imagem Principal */}
      <div
        className="relative h-80 sm:h-96 md:h-[450px] lg:h-[500px] xl:h-[600px] rounded-lg overflow-hidden shadow-lg bg-gray-100"
      ...
      >
        ...
        {/* Botões de Navegação - Ocultos em telas muito pequenas */}
        {images.length > 1 && (
          <>
            <button
              onClick={prevImage}
        ...
        <div className="flex gap-1.5 sm:gap-2 mt-3 sm:mt-4 overflow-x-auto pb-2 scrollbar-hide">
          {images.map((image, index) => (
            <button
              key={index}
              onClick={() => goToImage(index)}
              className={`relative flex-shrink-0 w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24 lg:w-28 lg:h-28 rounded-md overflow-hidden border-2 transition-colors touch-manipulation ${
